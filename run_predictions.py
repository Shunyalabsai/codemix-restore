#!/usr/bin/env python3
"""Run codemix_restore on ShunyaLabs ASR predictions and produce output + statistics.

Reads JSONL prediction files, applies ScriptRestorer to each prediction,
and writes results with statistics.
"""
...
LANG_CODE_MAP = {
    ...
    # Non-scheduled but present in data
...
OUTPUT_FILE = "shunyalabs_restored_xlit.jsonl"
STATS_FILE = "shunyalabs_restore_stats_xlit.json"

# Set to 0 for unlimited, or a positive number to limit records processed
MAX_RECORDS = int(os.environ.get("MAX_RECORDS", "0"))


def has_english(text: str) -> bool:
    """Check if text contains any ASCII English words (length >= 2)."""
    for word in text.split():
        if word.isascii() and any(c.isalpha() for c in word) and len(word) >= 2:
            return True
    return False


def count_english_words(text: str) -> int:
    """Count ASCII English words in text."""
    count = 0
    for word in text.split():
        cleaned = word.strip(".,!?;:'\"()[]{}—–-")
...
def count_indic_words(text: str) -> int:
    """Count non-ASCII (Indic) words in text."""
    count = 0
    for word in text.split():
        if not word.isascii():
            count += 1
...
def main():
    ...
    stats = defaultdict(lambda: {
        "total": 0,
        "modified": 0,  # predictions that were changed
        "unchanged": 0,
        "skipped": 0,  # unsupported language
        "english_words_found": 0,
        "total_words_processed": 0,
        "errors": 0,
        "sample_modifications": [],  # first N examples of changes
    })
        ...
        with open(filepath, encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                ...
                if changed:
                    ...
                    total_modified += 1
                    if len(lang_stats["sample_modifications"]) < 20:
                ...
                if total_processed % 10000 == 0 and total_processed > 0:
                    ...
                    print(f"  Processed {total_processed:,} ({rate:.0f}/s) | "
                          f"Modified: {total_modified:,} ({total_modified/total_processed*100:.1f}%)")

        print(f"  Finished {input_file} in {time.time() - file_start:.1f}s")
        if MAX_RECORDS > 0 and total_processed >= MAX_RECORDS:
            break
    ...
    # Print sample modifications per language
    print(f"\n{'='*90}")
    ...
    for lang_name, s in stats.items():
        stats_out[lang_name] = {k: v for k, v in s.items()}
